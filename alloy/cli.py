...
# =========================================================================
# Command 2: alloy search <query> (Local Offline Query)
# =========================================================================
...
def search(query: str):
    ...
    try:
        ...
        typer.secho("💡 Hint: Have you run 'alloy update' to fetch the index first?", fg=typer.colors.BLUE)
        raise typer.Exit(code=1)
...
def install(
    ...
    """
    try:
        # Step 1: Parse the recipe (Detect if local file path or remote API target) [3]
        ...
        if path_target.is_file():
            ...
            typer.secho(f"⏳ Fetching recipe for '{package}' from remote registry...", fg=typer.colors.CYAN)
